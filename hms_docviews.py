from django.shortcuts import render,redirect,HttpResponse # Used for rendering templates,redirecting and Handling HTTP response                                                                            # type: ignore
from django.contrib.auth.decorators import login_required # Restrict access to view based on authentication                                                                         # type: ignore
from dasapp.models import DoctorReg,Specialization,CustomUser,Appointment,AddPatient,MedicalHistory # Importing models from the app 'dsapp'                                 # type: ignore
from django.contrib import messages # Used for displaying success/error message in templates                                                                                             # type: ignore
...
def Patient_Appointment_Details_Remark(request):
    ...
    return render(request,'doc/view_appointment.html',context)                                                                     # type: ignore
...
def Patient_Appointment_Prescription(request):
    if request.method == 'POST':
        ...
        messages.success(request,"Status Update successfully")
        return redirect('view_appointment')
    return render(request,'doc/patient_list_app_appointment.html',context)                                                        # type: ignore
