...
here for eeach example add the ouput of qury like ouput table how it look like so that I can understand this my table and this is how my ouput for each example belowlook like 
...
Here are the output tables for each SQL query using your database schema and sample data:

## Query Outputs

### Q1: Total number of employees in each department and their average salary
```sql
...
### Q2: Departments where average salary is greater than $50,000
```sql
SELECT department_id, AVG(salary) as avg_salary
...
HAVING SUM(salary) = (SELECT MAX(dept_total) FROM (SELECT SUM(salary) as dept_total FROM employees GROUP BY department_id));
```
...
### Q4: INNER JOIN - Employee names with department names
```sql
SELECT e.first_name, e.last_name, d.department_name
...
| Beena      | Menon     | Finance         |
| Chetan     | Desai     | Finance         |
| Deepa      | Kaur      | HR              |
| Ekta       | Patel     | Sales           |
| Gopal      | Nair      | Engineering     |
| Harsha     | Roy       | Finance         |
| Imran      | Khan      | Sales           |

### Q5: LEFT JOIN - All departments and employees
```sql
SELECT d.department_name, e.first_name, e.last_name
FROM departments d LEFT JOIN employees e ON d.department_id = e.department_id;
```
...
|----------------|------------|-----------|
| Engineering    | Anil       | Sharma    |
| Engineering    | Gopal      | Nair      |
| Finance        | Beena      | Menon     |
| Finance        | Chetan     | Desai     |
| Finance        | Harsha     | Roy       |
| Sales          | Ekta       | Patel     |
| Sales          | Imran      | Khan      |
| HR             | Deepa      | Kaur      |

### Q6: SELF JOIN - Employees and their managers
```sql
SELECT e1.first_name || ' ' || e1.last_name as Employee,
       e2.first_name || ' ' || e2.last_name as Manager
...
| Employee     | Manager    |
|-------------|------------|
| Anil Sharma | Gopal Nair |
| Beena Menon | Harsha Roy |
| Chetan Desai| Harsha Roy |
| Deepa Kaur  | Gopal Nair |
| Ekta Patel  | Imran Khan |
| Gopal Nair  | NULL       |
| Harsha Roy  | NULL       |
| Imran Khan  | NULL       |

### Q8: Order details with product information
```sql
SELECT od.order_id, p.product_name, od.quantity, od.unit_price
...
### Q9: Employee emails with type
```sql
SELECT email as contact_email, 'Employee' as type FROM employees WHERE email IS NOT NULL;
```

| contact_email            | type     |
|--------------------------|----------|
| anil.sharma@example.com  | Employee |
| beena.menon@example.com  | Employee |
| chetan.desai@example.com | Employee |
| deepa.kaur@example.com   | Employee |
| ekta.patel@example.com   | Employee |
| gopal.nair@example.com   | Employee |
| harsha.roy@example.com   | Employee |
| imran.khan@example.com   | Employee |

### Q12: Top 3 customers with highest total order amounts
```sql
SELECT c.customer_name, SUM(o.order_amount) as total_spent
FROM customers c INNER JOIN orders o ON c.customer_id = o.customer_id
GROUP BY c.customer_id, c.customer_name ORDER BY total_spent DESC LIMIT 3;
```
...
### Q13: Employees with salary above department average
```sql
...
### Q14: Departments with more than 2 employees and average salary > 45000
```sql
SELECT d.department_name, COUNT(e.employee_id) as emp_count, AVG(e.salary) as avg_salary
FROM departments d INNER JOIN employees e ON d.department_id = e.department_id
GROUP BY d.department_id, d.department_name
HAVING COUNT(e.employee_id) > 2 AND AVG(e.salary) > 45000;
```
...
These output tables show exactly how each query result would appear when executed against your sample database. Each table demonstrates different SQL concepts like JOINs, aggregations, grouping, and filtering conditions working with your specific data.
